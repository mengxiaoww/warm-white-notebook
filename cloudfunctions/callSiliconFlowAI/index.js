...
- **重要**：中性粒细胞应记录 **NEUT#（绝对计数）** 而非 NEUT%（百分比）
  - 正确示例：NEUT# = 0.82 → neut: 0.82 ✅
  - 错误示例：NEUT% = 52.3% → neut: 52.3 ❌（这是百分比，不是绝对计数）
...
**neut字段说明**：
- 如果报告中有 NEUT# = 0.82，NEUT% = 52.3%
- 你必须输出 "neut": 0.82（取NEUT#的值）
- 绝不能输出 "neut": 52.3（这是百分比，不对）
- 判断方法：NEUT#的值是小数（0.x-8.x），NEUT%的值是整数百分比（30-70）
