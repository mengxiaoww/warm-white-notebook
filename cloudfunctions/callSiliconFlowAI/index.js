...
- values: 指标键值对（键名必须小写，如wbc、plt、hgb、neut、alt、ast等）
  - **⚠️ 重要**：数值必须保留用户原始输入的精度，不要四舍五入或去除小数！
  - 用户说"4.2"就输出4.2，用户说"70"就输出70
  - 小数值输出示例：4.2、3.6、2.8（保留小数点）
  - 整数值输出示例：70、120、80（不加.0）
...
- **数值精度要求**：严格按用户说的数值输出，有小数保留小数，整数就是整数！`,
