...
- 首页（`pages/home`）
  ...
  - **智能健康图谱数据可视化**：
    - **动态图表显示**：根据用户在配置页面勾选的记录项自动过滤图表分组，只显示启用的健康数据图表
    - **默认配置支持**：用户未自定义时使用默认配置（血常规、肝功能、肾功能、病毒学、酶类、生命体征等）
    - **实时更新**：配置变更后图表实时刷新，确保显示内容与用户需求一致
    - 支持7种图表分组：血常规、肝功能、肾功能、病毒指标、酶类指标、生命体征、生活指标
    - 饮食图表优化：从"热量"改为"每日饮食记录次数"，更符合实际使用场景
  - 一键进入各类记录功能
