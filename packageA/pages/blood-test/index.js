...
3. 识别各种口语和书面表达：
   - "点"代表小数点："五点二"="5.2"
   - 数字表达："一百三十四"="134"、"两百五"="250"
   - 小数表达："五点二"="5.2"、"四点八"="4.8"
   - 支持混合表达："白细胞5点2"、"血红蛋白134"

4. 单位自动匹配：
   - 自动识别常见单位：×10⁹/L、g/L、mg/L、%等
   - 如果语音中没有单位,根据指标配置自动补充
...
3. 智能匹配指标名称（支持多种表达）：
   ...
   - 血红蛋白 = HGB = Hb = 血色素
   - 血小板 = PLT = 血小板计数
   - 中性粒细胞 = NEUT = 中性粒 = 中性细胞 = NEUT#
   - 单核细胞 = MONO = 单核 = 单个核细胞 = MONO#
   - 淋巴细胞 = LYMPH = 淋巴 = 淋巴细胞数 = LYMPH#
   - C反应蛋白 = CRP = C-反应蛋白 = 超敏C反应蛋白
   - 红细胞 = RBC = 红细胞计数
   - 血细胞比容 = HCT = 压积
4. **重要**：跳过百分比指标（如NEUT%、LYMPH%等），只提取绝对值指标（如NEUT#、LYMPH#）
