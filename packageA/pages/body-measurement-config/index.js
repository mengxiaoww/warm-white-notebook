...
Page({
  data: {
    ...
    // 必选指标（不能取消勾选）
    requiredIndicators: ['height', 'weight'],
  ...
  handleBloodTestDataDeleted() {
    const app = getApp();
    if (app.globalData && app.globalData.bodyMeasurementDataDeleted) {
      const deleteInfo = app.globalData.bodyMeasurementDataDeleted;
      ...
      if (!isValid) {
        ...
        delete app.globalData.bodyMeasurementDataDeleted;
        return;
      }
  ...
  async loadIndicatorConfig() {
    ...
    try {
      ...
      if (editingDate) {
        // 🔧 优先查询当前日期的专用配置
        configRes = await db.collection('bodyMeasurementIndicatorConfig')
          .where({
            openid: openid,
        ...
        if (configRes.data.length === 0 && this.data.dateType === 'future') {
          ...
          // 查找最近的有效配置（包括今日和历史配置）
          const inheritableConfigRes = await db.collection('bodyMeasurementIndicatorConfig')
            .where({
              openid: openid,
              profileId: currentProfileId,
      ...
      let customSettings = {};
      try {
        const settingsRes = await db.collection('bodyMeasurementIndicatorSettings')
          .where({
            openid: openid,
            profileId: currentProfileId
          })
  ...
  async loadCustomIndicators() {
    ...
    try {
      const db = wx.cloud.database();
      const res = await db.collection('bodyMeasurementIndicators')
        .where({
          openid: openid,
          profileId: currentProfileId
        })
  ...
  // 返回方法 - 立即同步配置到身高体重页面预览
  async goBack() {


    // 立即同步当前配置状态到全局数据，供身高体重页面预览
  ...
  async updateCustomIndicatorStatus(indicatorId, isActive) {
    try {
      const db = wx.cloud.database();
      await db.collection('bodyMeasurementIndicators')
        .doc(indicatorId)
        .update({
          data: {
            isActive: isActive,
  ...
  async saveConfigSimple() {
    ...
    try {
      ...
      // 1. 获取旧配置，找出被取消勾选的指标
      const existingRes = await db.collection('bodyMeasurementIndicatorConfig')
        .where({
          openid: openid,
          profileId: currentProfileId,
          date: editingDate
      ...
      for (const indicatorId of removedIndicators) {
        ...
        if (settingsRes.data.length > 0) {
          for (const setting of settingsRes.data) {
            await db.collection('bodyMeasurementIndicatorSettings')
              .doc(setting._id)
              .remove();
          }
  ...
  async cleanupRemovedIndicatorData(removedIndicators, date) {
    ...
    try {
      ...
      // 查询该日期的身高体重数据
      const bloodRes = await db.collection('bodyMeasurements')
        .where({
          openid: openid,
          profileId: currentProfileId,
          date: date
  ...
  async saveConfig() {
    ...
    try {
      ...
      // 查询是否已有配置
      const res = await db.collection('bodyMeasurementIndicatorConfig')
        .where({
          openid: openid,
          profileId: currentProfileId
        })
      ...
      if (res.data.length > 0) {
        ...
        // 更新配置
        const updateResult = await db.collection('bodyMeasurementIndicatorConfig')
          .doc(res.data[0]._id)
          .update({
            data: configData
          });
        ...
        // 新建配置
        const addResult = await db.collection('bodyMeasurementIndicatorConfig').add({
          data: {
            openid,
            profileId: currentProfileId,
            selectedIndicators,
  ...
  async editIndicator(e) {
    ...
    try {
      ...
      if (openid && currentProfileId) {
        const db = wx.cloud.database();
        const res = await db.collection('bodyMeasurementIndicatorSettings')
          .where({
            openid: openid,
            profileId: currentProfileId,
            indicatorId: id
    ...
    try {
      ...
      if (isAddMode) {
        ...
        try {
          ...
          existingRes = await db.collection('bodyMeasurementIndicatorSettings')
            .where({
              openid: openid,
              profileId: currentProfileId,
              indicatorId: targetIndicatorId
      ...
      if (!collectionExists) {
        ...
        try {
          // 通过添加第一条记录来创建集合
          dbResult = await db.collection('bodyMeasurementIndicatorSettings').add({
            data: {
              openid: openid,
              profileId: currentProfileId,
              indicatorId: targetIndicatorId,
        ...
        try {
          dbResult = await db.collection('bodyMeasurementIndicatorSettings')
            .doc(existingRes.data[0]._id)
            .update({
              data: settingData
            });
