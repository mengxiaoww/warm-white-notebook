...
Page({
  ...
  restoreTemporaryUserInput() {
    ...
    if (tempInput.selectedDate === this.data.selectedDate) {
      console.log('🔄 恢复血糖用户临时输入:', tempInput.formData);
      ...
      console.log('✅ 血糖用户输入已恢复');
    } else {
      console.log('📅 血糖临时输入日期不匹配，忽略恢复');
    }
      ...
      for (let searchOffset = 0; searchOffset <= 5; searchOffset++) {
        ...
        if (fieldName === 'crp') {
          ...
          if (searchText.includes('检查项目') || searchText.includes('设备') ||
            ...
            searchText.includes('血球仪') || searchText.includes('全血糖') ||
  ...
  async saveData() {
    ...
    try {
      ...
      const existingRes = await db.collection('bloodSugars')
        .where({
          openid: openid,
      ...
      if (isNewRecord) {
        ...
        const res = await db.collection('bloodSugars').add({
          data: saveData
        });
        ...
        await db.collection('bloodSugars')
          .doc(targetRecordId)
          .update({
            data: updateData
          });
  ...
  async findInheritedConfig(targetDate, openid, currentProfileId) {
    try {
      ...
      const configRes = await db.collection('bloodSugarIndicatorConfig')
        .where({
          openid: openid,
          profileId: currentProfileId,
  ...
  async saveTemporaryConfigIfExists() {
    ...
    try {
      ...
      for (const indicator of newCustomIndicators) {
        // 检查是否已存在
        const existingRes = await db.collection('bloodSugarIndicatorSettings')
          .where({
            openid: openid,
            profileId: currentProfileId,
        ...
        if (existingRes.data.length === 0) {
          ...
          await db.collection('bloodSugarIndicatorSettings').add({
            data: {
              openid: openid,
              profileId: currentProfileId,
              indicatorId: indicator.id,
      ...
      // 查询现有配置
      const existingConfigRes = await db.collection('bloodSugarIndicatorConfig')
        .where({
          openid: openid,
          profileId: currentProfileId,
          date: selectedDate
      ...
      if (existingConfigRes.data.length > 0) {
        // 更新现有配置
        await db.collection('bloodSugarIndicatorConfig')
        ...
        // 创建新配置
        await db.collection('bloodSugarIndicatorConfig').add({
          data: {
            openid: openid,
            profileId: currentProfileId,
            date: selectedDate,
  ...
  async applyTodayConfigToFuture(todayDate, selectedIndicators, openid, currentProfileId, db) {
    try {
      // 查找所有未来日期的配置
      const futureConfigsRes = await db.collection('bloodSugarIndicatorConfig')
        .where({
          openid: openid,
          profileId: currentProfileId,
      ...
      const batchUpdatePromises = futureConfigsRes.data.map(config => {
        ...
        return db.collection('bloodSugarIndicatorConfig')
          .doc(config._id)
          .update({
            data: {
              selectedIndicators: selectedIndicators,
  ...
  async deleteRecord() {
    ...
    try {
      ...
      wx.showModal({
        title: '确认删除',
        content: `确定要删除 ${selectedDate} 的血糖记录吗？`,
        confirmText: '删除',
        cancelText: '取消',
        success: async (modalRes) => {
          ...
          if (modalRes.confirm) {
            try {
              // 1. 删除血糖记录
              await db.collection('bloodSugars').doc(actualRecordId).remove();
  ...
  async loadCompleteConfiguration() {
    ...
    try {
      ...
      const currentDateConfigRes = await db.collection('bloodSugarIndicatorConfig')
        .where({
          openid: openid,
          profileId: currentProfileId,
          date: selectedDate
      ...
      // 2. 查询当前日期的血糖数据
      console.log('🔍 步骤2：查询当前日期的血糖数据');
      console.log('🔍 查询参数确认:', { openid, profileId: currentProfileId, date: selectedDate });

      const bloodSugarRes = await db.collection('bloodSugars')
        .where({
          openid: openid,
          profileId: currentProfileId,
          date: selectedDate
      ...
      console.log('📋 血糖数据查询结果:', bloodSugarRes.data.length, '条记录');
      if (bloodSugarRes.data.length > 0) {
        console.log('📋 找到的数据:', bloodSugarRes.data[0]);
      }
      ...
      try {
        customSettingsRes = await db.collection('bloodSugarIndicatorSettings')
          .where({
            openid: openid,
            profileId: currentProfileId
          })
      ...
      console.log(`  - 血糖数据: ${bloodSugarRes.data ? bloodSugarRes.data.length : '查询失败'} 条`);
      ...
      if (!bloodSugarRes || !bloodSugarRes.data || !Array.isArray(bloodSugarRes.data)) {
        console.error('❌ 血糖数据查询结果格式异常:', bloodSugarRes);
        throw new Error('血糖数据查询结果格式异常');
      }
      ...
      if (currentDateConfigRes.data.length > 0) {
        ...
        } else if (bloodSugarRes.data.length > 0) {
          // 🔥 优先级2：如果没有专用配置但有数据，根据数据构建配置
          console.log('✅ 没有专用配置但有数据，根据数据构建配置');
          const bloodData = bloodSugarRes.data[0];
          console.log('📋 血糖数据字段:', Object.keys(bloodData));
      ...
      this.setData({
        bloodSugarIndicatorConfig: config,
  ...
  async loadTodayData() {
    ...
    console.log(`📊📊📊 开始加载 ${selectedDate} 的血糖数据 📊📊📊`);
    ...
    try {
      ...
      if (res.data.length > 0) {
        ...
        console.log(`❌ ${selectedDate} 没有血糖数据，开始清空表单`);

        console.log(`❌ ${selectedDate} 没有血糖数据，清空表单但保持配置`);
  ...
  async loadRecordData(recordId) {
    ...
    try {
      const db = wx.cloud.database();
      const res = await db.collection('bloodSugars').doc(recordId).get();
      ...
      const res = await wx.cloud.callFunction({
        ...
        data: {
          messages: [
            {
              role: 'user',
              content: `请从以下语音描述中提取血糖相关的检验数据：\n\n"${text}"\n\n请返回JSON格式：{ "indicators": [{ "id": "fbg", "label": "空腹血糖", "value": "数值", "unit": "mmol/L", "confidence": 90 }] }`
            }
          ],
          mode: 'unified'
        }
  ...
  fuzzyMatch(aiLabel, configName) {
    ...
    const bloodSugarKeywords = [
      ['bloodsugar', '血糖', 'glucose', 'glu', 'bg'],
      ['fbg', 'fpg', '空腹血糖', '空腹葡萄糖', 'fasting', '空腹'],
      ['pbg', '2hpg', '餐后血糖', '餐后2小时', 'postprandial', '餐后', '2h血糖'],
      ['hba1c', 'a1c', '糖化血红蛋白', 'glycated', '糖化'],
      ['rbg', '随机血糖', 'random', '任意', '随意'],
      ['ogtt', '糖耐量', 'tolerance', '糖耐']
    ];

    for (const keywords of bloodSugarKeywords) {
