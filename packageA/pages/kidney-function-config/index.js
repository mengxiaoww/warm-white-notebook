...
Page({
  data: {
    ...
    // 必选指标（不能取消勾选）- 针对移植后患者
    requiredIndicators: ['cr', 'bun', 'ua'],

    // 所有指标的配置信息（包括自定义设置）
    indicatorConfigs: {
      cr: { name: '肌酐', min: '44', max: '133', unit: 'μmol/L' },
      bun: { name: '尿素氮', min: '2.9', max: '8.2', unit: 'mmol/L' },
      ua: { name: '尿酸', min: '208', max: '428', unit: 'μmol/L' },
      egfr: { name: 'eGFR', min: '90', max: '120', unit: 'ml/min/1.73m²' },
      cysc: { name: '胱抑素C', min: '0.51', max: '1.09', unit: 'mg/L' },
      b2mg: { name: 'β2微球蛋白', min: '0.7', max: '1.8', unit: 'mg/L' },
      rtn: { name: '视黄醇结合蛋白', min: '20', max: '60', unit: 'mg/L' },
      upro: { name: '尿蛋白', min: '0', max: '150', unit: 'mg/L' },
      microalb: { name: '微量白蛋白', min: '0', max: '30', unit: 'mg/L' }
    },
  ...
  onLoad(options) {
    ...
    if (userInfoSuccess) {
      // 🔧 关键修复：检查是否有临时配置
      const app = getApp();
      if (app.globalData && app.globalData.temporaryKidneyFunctionConfig &&
        app.globalData.temporaryKidneyFunctionConfig.configDate === editingDate) {

        this.loadTemporaryKidneyConfigToPage();
      } else {
  ...
  onHide() {
    ...
    // 🔧 修复：确保在页面隐藏时同步配置到肾功能页面
    // 无论用户如何离开页面（点击返回按钮、手势返回、系统返回），都同步配置
    this.syncConfigToKidneyFunctionPage();
  },
  ...
  onShow() {
    ...
    // 🆕 检查是否有肾功能数据删除事件
    this.handleKidneyFunctionDataDeleted();
    ...
    const app = getApp();
    const hasTemporaryConfig = app.globalData && app.globalData.temporaryKidneyFunctionConfig && app.globalData.temporaryKidneyFunctionConfig.isTemporary;
  ...
  // 🆕 处理肾功能数据删除事件
  handleKidneyFunctionDataDeleted() {
    const app = getApp();
    if (app.globalData && app.globalData.kidneyFunctionDataDeleted) {
      const deleteInfo = app.globalData.kidneyFunctionDataDeleted;
      ...
      if (!isValid) {
        ...
        delete app.globalData.kidneyFunctionDataDeleted;
        return;
      }
  ...
  // 🔧 从临时配置恢复页面状态
  loadTemporaryKidneyConfigToPage() {
    const app = getApp();
    const tempConfig = app.globalData.temporaryKidneyFunctionConfig;
  ...
  async loadIndicatorConfig() {
    ...
    try {
      ...
      if (editingDate) {
        // 🔧 优先查询当前日期的专用配置
        configRes = await db.collection('kidneyFunctionConfig')
          .where({
            openid: openid,
        ...
        if (configRes.data.length === 0 && this.data.dateType === 'future') {
          ...
          // 查找最近的有效配置（包括今日和历史配置）
          const inheritableConfigRes = await db.collection('kidneyFunctionConfig')
            .where({
              openid: openid,
              profileId: currentProfileId,
      ...
      let customSettings = {};
      try {
        const settingsRes = await db.collection('kidneyFunctionSettings')
          .where({
            openid: openid,
            profileId: currentProfileId
          })
  ...
  async saveConfigSimple() {
    ...
    try {
      ...
      // 1. 获取旧配置，找出被取消勾选的指标
      const existingRes = await db.collection('kidneyFunctionConfig')
        .where({
          openid: openid,
          profileId: currentProfileId,
          date: editingDate
  ...
  async applyConfigToFuture(selectedIndicators) {
    ...
    try {
      ...
      // 查询所有未来日期的配置（大于今天的日期）
      const futureConfigsRes = await db.collection('kidneyFunctionConfig')
        .where({
          openid: openid,
          profileId: currentProfileId,
      ...
      const batchUpdatePromises = futureConfigsRes.data.map(config => {
        ...
        return db.collection('kidneyFunctionConfig')
          .doc(config._id)
          .update({
            data: {
              selectedIndicators: selectedIndicators,
      ...
      for (const indicatorId of removedIndicators) {
        ...
        if (settingsRes.data.length > 0) {
          for (const setting of settingsRes.data) {
            await db.collection('kidneyFunctionSettings')
              .doc(setting._id)
              .remove();
          }
  ...
  async cleanupRemovedIndicatorData(removedIndicators, date) {
    ...
    try {
      ...
      // 查询该日期的肾功能数据
      const kidneyRes = await db.collection('kidneyFunctionRecords')
        .where({
          openid: openid,
          profileId: currentProfileId,
          date: date
      ...
      if (kidneyRes.data.length > 0) {
        const kidneyData = kidneyRes.data[0];
        const updateData = { updateTime: db.serverDate() };
        let needUpdate = false;

        // 清理直接字段
        removedIndicators.forEach(indicatorId => {
          if (kidneyData[indicatorId] !== undefined) {
        ...
        if (kidneyData.customValues) {
          const newCustomValues = {};
          Object.keys(kidneyData.customValues).forEach(key => {
            if (!removedIndicators.includes(key)) {
              newCustomValues[key] = kidneyData.customValues[key];
            }
          });
        ...
        // 执行数据清理
        if (needUpdate) {
          await db.collection('kidneyFunctionRecords')
            .doc(kidneyData._id)
            .update({
              data: updateData
            });
  ...
  generateAutoShortName(fullName) {
    ...
    if (name.length === 1) {
      ...
      if (/[\u4e00-\u9fa5]/.test(name)) {
        ...
        if (name.includes('尿酸')) {
          shortName = '尿酸';
        } else if (name.includes('肌酐')) {
          shortName = '肌酐';
        } else if (name.includes('尿素')) {
          shortName = '尿素';
        } else if (name.includes('微球蛋白')) {
          shortName = name.substring(0, 2);
        } else if (name.includes('胱抑素')) {
          shortName = '胱抑';
        } else if (name.includes('结合蛋白')) {
          shortName = name.substring(0, 2);
        } else {
          // 通用策略：取前两个字符
          shortName = name.substring(0, 2);
  ...
  checkShortNameConflict(shortName) {
    ...
    // 预设指标的简称
    const presetShortNames = ['肌酐', '尿素', '尿酸', 'eGFR', '胱抑', 'β2微', '视黄'];
    ...
    try {
      ...
      if (isAddMode) {
        ...
        try {
          ...
          existingRes = await db.collection('kidneyFunctionSettings')
            .where({
              openid: openid,
              profileId: currentProfileId,
              indicatorId: targetIndicatorId
      ...
      if (!collectionExists) {
        ...
        try {
          // 通过添加第一条记录来创建集合
          dbResult = await db.collection('kidneyFunctionSettings').add({
            data: {
              openid: openid,
              profileId: currentProfileId,
              indicatorId: targetIndicatorId,
        ...
        try {
          dbResult = await db.collection('kidneyFunctionSettings')
            .doc(existingRes.data[0]._id)
            .update({
              data: settingData
            });
