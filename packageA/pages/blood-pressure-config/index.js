...
Page({
  ...
  handleBloodTestDataDeleted() {
    const app = getApp();
    if (app.globalData && app.globalData.bloodPressureDataDeleted) {
      const deleteInfo = app.globalData.bloodPressureDataDeleted;
      ...
      if (!isValid) {
        ...
        delete app.globalData.bloodPressureDataDeleted;
        return;
      }
  ...
  async loadIndicatorConfig() {
    ...
    try {
      ...
      if (editingDate) {
        // 🔧 优先查询当前日期的专用配置
        configRes = await db.collection('bloodPressureIndicatorConfig')
          .where({
            openid: openid,
        ...
        if (configRes.data.length === 0 && this.data.dateType === 'future') {
          ...
          // 查找最近的有效配置（包括今日和历史配置）
          const inheritableConfigRes = await db.collection('bloodPressureIndicatorConfig')
            .where({
              openid: openid,
              profileId: currentProfileId,
      ...
      let customSettings = {};
      try {
        const settingsRes = await db.collection('bloodPressureIndicatorSettings')
          .where({
            openid: openid,
            profileId: currentProfileId
          })
  ...
  async loadCustomIndicators() {
    ...
    try {
      const db = wx.cloud.database();
      const res = await db.collection('bloodPressureIndicators')
        .where({
          openid: openid,
          profileId: currentProfileId
        })
  ...
  async updateCustomIndicatorStatus(indicatorId, isActive) {
    try {
      const db = wx.cloud.database();
      await db.collection('bloodPressureIndicators')
        .doc(indicatorId)
        .update({
          data: {
            isActive: isActive,
  ...
  async saveConfigSimple() {
    ...
    try {
      ...
      // 1. 获取旧配置，找出被取消勾选的指标
      const existingRes = await db.collection('bloodPressureIndicatorConfig')
        .where({
          openid: openid,
          profileId: currentProfileId,
          date: editingDate
      ...
      for (const indicatorId of removedIndicators) {
        ...
        if (settingsRes.data.length > 0) {
          for (const setting of settingsRes.data) {
            await db.collection('bloodPressureIndicatorSettings')
              .doc(setting._id)
              .remove();
          }
  ...
  async cleanupRemovedIndicatorData(removedIndicators, date) {
    ...
    try {
      ...
      // 查询该日期的血压数据
      const bloodRes = await db.collection('bloodPressures')
        .where({
          openid: openid,
          profileId: currentProfileId,
          date: date
  ...
  async saveConfig() {
    ...
    try {
      ...
      // 查询是否已有配置
      const res = await db.collection('bloodPressureIndicatorConfig')
        .where({
          openid: openid,
          profileId: currentProfileId
        })
      ...
      if (res.data.length > 0) {
        ...
        // 更新配置
        const updateResult = await db.collection('bloodPressureIndicatorConfig')
          .doc(res.data[0]._id)
          .update({
            data: configData
          });
        ...
        // 新建配置
        const addResult = await db.collection('bloodPressureIndicatorConfig').add({
          data: {
            openid,
            profileId: currentProfileId,
            selectedIndicators,
  ...
  async editIndicator(e) {
    ...
    try {
      ...
      if (openid && currentProfileId) {
        const db = wx.cloud.database();
        const res = await db.collection('bloodPressureIndicatorSettings')
          .where({
            openid: openid,
            profileId: currentProfileId,
            indicatorId: id
    ...
    try {
      ...
      if (isAddMode) {
        ...
        try {
          ...
          existingRes = await db.collection('bloodPressureIndicatorSettings')
            .where({
              openid: openid,
              profileId: currentProfileId,
              indicatorId: targetIndicatorId
      ...
      if (!collectionExists) {
        ...
        try {
          // 通过添加第一条记录来创建集合
          dbResult = await db.collection('bloodPressureIndicatorSettings').add({
            data: {
              openid: openid,
              profileId: currentProfileId,
              indicatorId: targetIndicatorId,
        ...
        try {
          dbResult = await db.collection('bloodPressureIndicatorSettings')
            .doc(existingRes.data[0]._id)
            .update({
              data: settingData
            });
