...
Page({
  ...
  // 页面初始化
  onLoad(options) {
    console.log('🎯 血氧页面 onLoad，传入参数:', options);
  ...
  restoreTemporaryUserInput() {
    ...
    if (tempInput.selectedDate === this.data.selectedDate) {
      console.log('🔄 恢复血氧用户临时输入:', tempInput.formData);
      ...
      console.log('✅ 血氧用户输入已恢复');
    } else {
      console.log('📅 血氧临时输入日期不匹配，忽略恢复');
    }
  ...
  parseOCRResult(items) {
    ...
    const indicatorRules = {
      'fbg': {
        // 空腹血氧关键词
        keywords: ['FBG', 'FPG', '空腹血氧', '空腹葡萄糖', '空腹血氧（FBG）', '空腹血氧(FBG)', '★空腹血氧'],
        englishKeywords: ['FBG', 'FPG']
      },
      'pbg': {
        // 餐后2小时血氧关键词
        keywords: ['PBG', '2hPG', '餐后血氧', '餐后2小时血氧', '2h血氧', '餐后2h血氧', '★餐后血氧'],
      ...
      'rbg': {
        // 随机血氧关键词
        keywords: ['RBG', '随机血氧', '随机葡萄糖', '任意血氧', '★随机血氧'],
        englishKeywords: ['RBG']
      }
    };
    ...
    items.forEach((item, index) => {
      ...
      // 特别标记包含血氧相关内容的行
      if (text.includes('血氧') || text.includes('FBG') || text.includes('PBG') || text.includes('HbA1c')) {
        console.log(`    ⚠️ 此行包含血氧相关关键词`);
      }
    });
  ...
  shouldSkipPercentage(keyword, text) {
    // 血氧指标不需要跳过百分比,因为HbA1c本身就是百分比
    // 只需要确保不误匹配其他无关的百分比数据
    return false;
  },

  // 重新设计的数值提取逻辑 - 针对5张血氧图片优化
      ...
      for (let searchOffset = 0; searchOffset <= 5; searchOffset++) {
        ...
        if (fieldName === 'crp') {
          ...
          if (searchText.includes('检查项目') || searchText.includes('设备') ||
            ...
            searchText.includes('血球仪') || searchText.includes('全血氧') ||
  ...
  async saveData() {
    ...
    try {
      ...
      const existingRes = await db.collection('bloodOxygens')
        .where({
          openid: openid,
      ...
      if (isNewRecord) {
        ...
        const res = await db.collection('bloodOxygens').add({
          data: saveData
        });
        ...
        await db.collection('bloodOxygens')
          .doc(targetRecordId)
          .update({
            data: updateData
          });
  ...
  async findInheritedConfig(targetDate, openid, currentProfileId) {
    try {
      ...
      const configRes = await db.collection('bloodOxygenIndicatorConfig')
        .where({
          openid: openid,
          profileId: currentProfileId,
  ...
  async saveTemporaryConfigIfExists() {
    ...
    try {
      ...
      for (const indicator of newCustomIndicators) {
        // 检查是否已存在
        const existingRes = await db.collection('bloodOxygenIndicatorSettings')
          .where({
            openid: openid,
            profileId: currentProfileId,
        ...
        if (existingRes.data.length === 0) {
          ...
          await db.collection('bloodOxygenIndicatorSettings').add({
            data: {
              openid: openid,
              profileId: currentProfileId,
              indicatorId: indicator.id,
      ...
      // 查询现有配置
      const existingConfigRes = await db.collection('bloodOxygenIndicatorConfig')
        .where({
          openid: openid,
          profileId: currentProfileId,
          date: selectedDate
      ...
      if (existingConfigRes.data.length > 0) {
        // 更新现有配置
        await db.collection('bloodOxygenIndicatorConfig')
        ...
        // 创建新配置
        await db.collection('bloodOxygenIndicatorConfig').add({
          data: {
            openid: openid,
            profileId: currentProfileId,
            date: selectedDate,
  ...
  async applyTodayConfigToFuture(todayDate, selectedIndicators, openid, currentProfileId, db) {
    try {
      // 查找所有未来日期的配置
      const futureConfigsRes = await db.collection('bloodOxygenIndicatorConfig')
        .where({
          openid: openid,
          profileId: currentProfileId,
      ...
      const batchUpdatePromises = futureConfigsRes.data.map(config => {
        ...
        return db.collection('bloodOxygenIndicatorConfig')
          .doc(config._id)
          .update({
            data: {
              selectedIndicators: selectedIndicators,
  ...
  async deleteRecord() {
    ...
    try {
      ...
      wx.showModal({
        title: '确认删除',
        content: `确定要删除 ${selectedDate} 的血氧记录吗？`,
        confirmText: '删除',
        cancelText: '取消',
        success: async (modalRes) => {
          ...
          if (modalRes.confirm) {
            try {
              // 1. 删除血氧记录
              await db.collection('bloodOxygens').doc(actualRecordId).remove();
  ...
  async loadCompleteConfiguration() {
    ...
    try {
      ...
      const currentDateConfigRes = await db.collection('bloodOxygenIndicatorConfig')
        .where({
          openid: openid,
          profileId: currentProfileId,
          date: selectedDate
      ...
      // 2. 查询当前日期的血氧数据
      console.log('🔍 步骤2：查询当前日期的血氧数据');
      console.log('🔍 查询参数确认:', { openid, profileId: currentProfileId, date: selectedDate });

      const bloodOxygenRes = await db.collection('bloodOxygens')
        .where({
          openid: openid,
          profileId: currentProfileId,
          date: selectedDate
      ...
      console.log('📋 血氧数据查询结果:', bloodOxygenRes.data.length, '条记录');
      if (bloodOxygenRes.data.length > 0) {
        console.log('📋 找到的数据:', bloodOxygenRes.data[0]);
      }
      ...
      try {
        customSettingsRes = await db.collection('bloodOxygenIndicatorSettings')
          .where({
            openid: openid,
            profileId: currentProfileId
          })
      ...
      console.log(`  - 血氧数据: ${bloodOxygenRes.data ? bloodOxygenRes.data.length : '查询失败'} 条`);
      ...
      if (!bloodOxygenRes || !bloodOxygenRes.data || !Array.isArray(bloodOxygenRes.data)) {
        console.error('❌ 血氧数据查询结果格式异常:', bloodOxygenRes);
        throw new Error('血氧数据查询结果格式异常');
      }
      ...
      if (currentDateConfigRes.data.length > 0) {
        ...
        } else if (bloodOxygenRes.data.length > 0) {
          // 🔥 优先级2：如果没有专用配置但有数据，根据数据构建配置
          console.log('✅ 没有专用配置但有数据，根据数据构建配置');
          const bloodData = bloodOxygenRes.data[0];
          console.log('📋 血氧数据字段:', Object.keys(bloodData));
      ...
      this.setData({
        bloodOxygenIndicatorConfig: config,
  ...
  async loadTodayData() {
    ...
    console.log(`📊📊📊 开始加载 ${selectedDate} 的血氧数据 📊📊📊`);
    ...
    try {
      ...
      if (res.data.length > 0) {
        ...
        console.log(`❌ ${selectedDate} 没有血氧数据，开始清空表单`);

        console.log(`❌ ${selectedDate} 没有血氧数据，清空表单但保持配置`);
  ...
  async loadRecordData(recordId) {
    ...
    try {
      const db = wx.cloud.database();
      const res = await db.collection('bloodOxygens').doc(recordId).get();
