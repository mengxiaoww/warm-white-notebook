  ...
  async saveTemporaryConfigIfExists() {
    ...
    try {
      ...
      for (const indicator of newCustomIndicators) {
        // 检查是否已存在
        const existingRes = await db.collection('waterIntakeIndicators')
          .where({
            openid: openid,
            profileId: currentProfileId,
            id: indicator.id
        ...
        if (existingRes.data.length === 0) {
          // 新增自定义指标
          await db.collection('waterIntakeIndicators').add({
            data: {
              openid: openid,
              profileId: currentProfileId,
              id: indicator.id,
      ...
      if (res.data.length > 0) {
        ...
        allConfiguredIds.forEach(indicatorId => {
          if (formData[indicatorId] === undefined) {
            // 🔥 修复：正确处理0值，不能用 || 运算符
            // 🔧 兼容旧字段名
            const fieldValue = (data[indicatorId] || data['waterIntake']);
