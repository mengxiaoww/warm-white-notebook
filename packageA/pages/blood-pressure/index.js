...
Page({
  ...
  // 页面初始化
  onLoad(options) {
    console.log('🎯 血压页面 onLoad，传入参数:', options);
  ...
  restoreTemporaryUserInput() {
    ...
    if (tempInput.selectedDate === this.data.selectedDate) {
      console.log('🔄 恢复血压用户临时输入:', tempInput.formData);
      ...
      console.log('✅ 血压用户输入已恢复');
    } else {
      console.log('📅 血压临时输入日期不匹配，忽略恢复');
    }
  ...
  parseOCRResult(items) {
    ...
    const indicatorRules = {
      'fbg': {
        // 空腹血压关键词
        keywords: ['FBG', 'FPG', '空腹血压', '空腹葡萄糖', '空腹血压（FBG）', '空腹血压(FBG)', '★空腹血压'],
        englishKeywords: ['FBG', 'FPG']
      },
      'pbg': {
        // 餐后2小时血压关键词
        keywords: ['PBG', '2hPG', '餐后血压', '餐后2小时血压', '2h血压', '餐后2h血压', '★餐后血压'],
      ...
      'rbg': {
        // 随机血压关键词
        keywords: ['RBG', '随机血压', '随机葡萄糖', '任意血压', '★随机血压'],
        englishKeywords: ['RBG']
      }
    };
    ...
    items.forEach((item, index) => {
      ...
      // 特别标记包含血压相关内容的行
      if (text.includes('血压') || text.includes('FBG') || text.includes('PBG') || text.includes('HbA1c')) {
        console.log(`    ⚠️ 此行包含血压相关关键词`);
      }
    });
  ...
  shouldSkipPercentage(keyword, text) {
    // 血压指标不需要跳过百分比,因为HbA1c本身就是百分比
    // 只需要确保不误匹配其他无关的百分比数据
    return false;
  },

  // 重新设计的数值提取逻辑 - 针对5张血压图片优化
      ...
      for (let searchOffset = 0; searchOffset <= 5; searchOffset++) {
        ...
        if (fieldName === 'crp') {
          ...
          if (searchText.includes('检查项目') || searchText.includes('设备') ||
            ...
            searchText.includes('血球仪') || searchText.includes('全血压') ||
  ...
  configIndicators() {
    ...
    // 🔧 重要：传递当前日期给配置页面
    wx.navigateTo({
      url: `/packageA/pages/blood-pressure-config/index?date=${selectedDate}`,
      fail: (err) => {
  ...
  viewHistory() {
    wx.navigateTo({
      url: '/pages/blood-oxygen-history/index'
    });
  },
  ...
  async saveData() {
    ...
    try {
      ...
      const existingRes = await db.collection('bloodPressures')
        .where({
          openid: openid,
      ...
      if (isNewRecord) {
        ...
        const res = await db.collection('bloodPressures').add({
          data: saveData
        });
        ...
        await db.collection('bloodPressures')
          .doc(targetRecordId)
          .update({
            data: updateData
          });
  ...
  async findInheritedConfig(targetDate, openid, currentProfileId) {
    try {
      ...
      const configRes = await db.collection('bloodPressureIndicatorConfig')
        .where({
          openid: openid,
          profileId: currentProfileId,
  ...
  async saveTemporaryConfigIfExists() {
    ...
    try {
      ...
      for (const indicator of newCustomIndicators) {
        // 检查是否已存在
        const existingRes = await db.collection('bloodPressureIndicatorSettings')
          .where({
            openid: openid,
            profileId: currentProfileId,
        ...
        if (existingRes.data.length === 0) {
          ...
          await db.collection('bloodPressureIndicatorSettings').add({
            data: {
              openid: openid,
              profileId: currentProfileId,
              indicatorId: indicator.id,
      ...
      // 查询现有配置
      const existingConfigRes = await db.collection('bloodPressureIndicatorConfig')
        .where({
          openid: openid,
          profileId: currentProfileId,
          date: selectedDate
      ...
      if (existingConfigRes.data.length > 0) {
        // 更新现有配置
        await db.collection('bloodPressureIndicatorConfig')
        ...
        // 创建新配置
        await db.collection('bloodPressureIndicatorConfig').add({
          data: {
            openid: openid,
            profileId: currentProfileId,
            date: selectedDate,
  ...
  async applyTodayConfigToFuture(todayDate, selectedIndicators, openid, currentProfileId, db) {
    try {
      // 查找所有未来日期的配置
      const futureConfigsRes = await db.collection('bloodPressureIndicatorConfig')
        .where({
          openid: openid,
          profileId: currentProfileId,
      ...
      const batchUpdatePromises = futureConfigsRes.data.map(config => {
        ...
        return db.collection('bloodPressureIndicatorConfig')
          .doc(config._id)
          .update({
            data: {
              selectedIndicators: selectedIndicators,
  ...
  async deleteRecord() {
    ...
    try {
      ...
      wx.showModal({
        title: '确认删除',
        content: `确定要删除 ${selectedDate} 的血压记录吗？`,
        confirmText: '删除',
        cancelText: '取消',
        success: async (modalRes) => {
          ...
          if (modalRes.confirm) {
            try {
              // 1. 删除血压记录
              await db.collection('bloodPressures').doc(actualRecordId).remove();
  ...
  async loadCompleteConfiguration() {
    ...
    try {
      ...
      const currentDateConfigRes = await db.collection('bloodPressureIndicatorConfig')
        .where({
          openid: openid,
          profileId: currentProfileId,
          date: selectedDate
      ...
      // 2. 查询当前日期的血压数据
      console.log('🔍 步骤2：查询当前日期的血压数据');
      console.log('🔍 查询参数确认:', { openid, profileId: currentProfileId, date: selectedDate });

      const bloodPressureRes = await db.collection('bloodPressures')
        .where({
          openid: openid,
          profileId: currentProfileId,
          date: selectedDate
      ...
      console.log('📋 血压数据查询结果:', bloodPressureRes.data.length, '条记录');
      if (bloodPressureRes.data.length > 0) {
        console.log('📋 找到的数据:', bloodPressureRes.data[0]);
      }
      ...
      try {
        customSettingsRes = await db.collection('bloodPressureIndicatorSettings')
          .where({
            openid: openid,
            profileId: currentProfileId
          })
      ...
      console.log(`  - 血压数据: ${bloodPressureRes.data ? bloodPressureRes.data.length : '查询失败'} 条`);
      ...
      if (!bloodPressureRes || !bloodPressureRes.data || !Array.isArray(bloodPressureRes.data)) {
        console.error('❌ 血压数据查询结果格式异常:', bloodPressureRes);
        throw new Error('血压数据查询结果格式异常');
      }
      ...
      if (currentDateConfigRes.data.length > 0) {
        ...
        } else if (bloodPressureRes.data.length > 0) {
          // 🔥 优先级2：如果没有专用配置但有数据，根据数据构建配置
          console.log('✅ 没有专用配置但有数据，根据数据构建配置');
          const bloodData = bloodPressureRes.data[0];
          console.log('📋 血压数据字段:', Object.keys(bloodData));
      ...
      const defaultIndicators = {
        systolic: { name: '高压', unit: 'mmHg' }, diastolic: { name: '低压', unit: 'mmHg' },
      ...
      if (basicIndicators.length === 0 && customIndicators.length === 0) {
        ...
        const fallbackIndicators = [
          { id: 'systolic', name: '高压', min: '90', max: '140', unit: 'mmHg' }, { id: 'diastolic', name: '低压', min: '60', max: '90', unit: 'mmHg' }
        ];
        ...
        this.setData({
          bloodPressureIndicatorConfig: { systolic: true, diastolic: true },
      ...
      this.setData({
        bloodPressureIndicatorConfig: config,
      ...
      const defaultIndicators = [
        { id: 'systolic', name: '高压', min: '90', max: '140', unit: 'mmHg' },
        { id: 'diastolic', name: '低压', min: '60', max: '90', unit: 'mmHg' }
      ];
  ...
  async loadTodayData() {
    ...
    console.log(`📊📊📊 开始加载 ${selectedDate} 的血压数据 📊📊📊`);
    ...
    try {
      ...
      if (res.data.length > 0) {
        ...
        console.log(`❌ ${selectedDate} 没有血压数据，开始清空表单`);

        console.log(`❌ ${selectedDate} 没有血压数据，清空表单但保持配置`);
  ...
  async loadRecordData(recordId) {
    ...
    try {
      const db = wx.cloud.database();
      const res = await db.collection('bloodPressures').doc(recordId).get();
      ...
      const res = await wx.cloud.callFunction({
        ...
        data: {
          messages: [
            {
              role: 'user',
              content: `请从以下语音描述中提取血压相关的检验数据：\n\n"${text}"\n\n请返回JSON格式：{ "indicators": [{ "id": "systolic", "label": "收缩压", "value": "数值", "unit": "mmHg", "confidence": 90 }] }`
            }
          ],
          mode: 'unified'
        }
  ...
  fuzzyMatch(aiLabel, configName) {
    ...
    const bloodPressureKeywords = [
      ['systolic', 'sbp', 'high', '收缩压', '高压', '上压'],
      ['diastolic', 'dbp', 'low', '舒张压', '低压', '下压']
    ];

    for (const keywords of bloodPressureKeywords) {
