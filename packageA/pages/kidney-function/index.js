...
Page({
  ...
  uploadImageToCloud(imagePath) {
    return new Promise((resolve, reject) => {
      const cloudPath = `kidney-images/${Date.now()}-${Math.random().toString(36).slice(2)}.jpg`;
      wx.cloud.uploadFile({
        cloudPath: cloudPath,
        filePath: imagePath,
...
2. 🚨严禁编造数据：如果某指标在图片中找不到，绝对不要输出该指标，宁可少返回也不能编造
3. 每个value必须是你从图片中亲眼看到的数字，不能推算、估计或凭医学知识补充
4. 如果图片模糊看不清某个值，直接跳过该指标，不要猜测
5. 指标名前的*号是异常标记，忽略它，按正常名称匹配
