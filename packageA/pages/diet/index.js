...
Page({
  ...
  // 页面初始化
  onLoad(options) {
    console.log('🎯 饮食页面 onLoad，传入参数:', options);
  ...
  restoreTemporaryUserInput() {
    ...
    if (tempInput.selectedDate === this.data.selectedDate) {
      console.log('🔄 恢复饮食用户临时输入:', tempInput.formData);
      ...
      console.log('✅ 饮食用户输入已恢复');
    } else {
      console.log('📅 饮食临时输入日期不匹配，忽略恢复');
    }
  ...
  parseOCRResult(items) {
    ...
    items.forEach((item, index) => {
      ...
      // 特别标记包含饮食相关内容的行
      if (text.includes('饮食') || text.includes('FBG') || text.includes('PBG') || text.includes('HbA1c')) {
        console.log(`    ⚠️ 此行包含饮食相关关键词`);
      }
    });
  ...
  shouldSkipPercentage(keyword, text) {
    // 饮食指标不需要跳过百分比,因为HbA1c本身就是百分比
    // 只需要确保不误匹配其他无关的百分比数据
    return false;
  },

  // 重新设计的数值提取逻辑 - 针对5张饮食图片优化
      ...
      for (let searchOffset = 0; searchOffset <= 5; searchOffset++) {
        ...
        if (fieldName === 'crp') {
          ...
          if (searchText.includes('检查项目') || searchText.includes('设备') ||
            ...
            searchText.includes('血球仪') || searchText.includes('全饮食') ||
  ...
  configIndicators() {
    ...
    // 🔧 重要：传递当前日期给配置页面
    wx.navigateTo({
      url: `/packageA/pages/diet-config/index?date=${selectedDate}`,
      fail: (err) => {
  ...
  async saveData() {
    ...
    try {
      ...
      const existingRes = await db.collection('diets')
        .where({
          openid: openid,
      ...
      if (isNewRecord) {
        ...
        const res = await db.collection('diets').add({
          data: saveData
        });
  ...
  async findInheritedConfig(targetDate, openid, currentProfileId) {
    try {
      ...
      const configRes = await db.collection('dietIndicatorConfig')
        .where({
          openid: openid,
          profileId: currentProfileId,
  ...
  async saveTemporaryConfigIfExists() {
    ...
    try {
      ...
      // 查询现有配置
      const existingConfigRes = await db.collection('dietIndicatorConfig')
        .where({
          openid: openid,
          profileId: currentProfileId,
          date: selectedDate
      ...
      if (existingConfigRes.data.length > 0) {
        // 更新现有配置
        await db.collection('dietIndicatorConfig')
        ...
        // 创建新配置
        await db.collection('dietIndicatorConfig').add({
          data: {
            openid: openid,
            profileId: currentProfileId,
            date: selectedDate,
  ...
  async applyTodayConfigToFuture(todayDate, selectedIndicators, openid, currentProfileId, db) {
    try {
      // 查找所有未来日期的配置
      const futureConfigsRes = await db.collection('dietIndicatorConfig')
        .where({
          openid: openid,
          profileId: currentProfileId,
      ...
      const batchUpdatePromises = futureConfigsRes.data.map(config => {
        ...
        return db.collection('dietIndicatorConfig')
          .doc(config._id)
          .update({
            data: {
              selectedIndicators: selectedIndicators,
  ...
  async deleteRecord() {
    ...
    try {
      ...
      wx.showModal({
        title: '确认删除',
        content: `确定要删除 ${selectedDate} 的饮食记录吗？`,
        confirmText: '删除',
        cancelText: '取消',
        success: async (modalRes) => {
          ...
          if (modalRes.confirm) {
            try {
              // 1. 删除饮食记录
              await db.collection('diets').doc(actualRecordId).remove();
  ...
  async loadCompleteConfiguration() {
    ...
    try {
      ...
      const currentDateConfigRes = await db.collection('dietIndicatorConfig')
        .where({
          openid: openid,
          profileId: currentProfileId,
          date: selectedDate
      ...
      console.log('📋 饮食数据查询结果:', dietRes.data.length, '条记录');
      if (dietRes.data.length > 0) {
        console.log('📋 找到的数据:', dietRes.data[0]);
      }
      ...
      try {
        customSettingsRes = await db.collection('dietIndicatorSettings')
          .where({
            openid: openid,
            profileId: currentProfileId
          })
      ...
      console.log(`  - 饮食数据: ${dietRes.data ? dietRes.data.length : '查询失败'} 条`);
      ...
      if (!dietRes || !dietRes.data || !Array.isArray(dietRes.data)) {
        console.error('❌ 饮食数据查询结果格式异常:', dietRes);
        throw new Error('饮食数据查询结果格式异常');
      }
      ...
      if (currentDateConfigRes.data.length > 0) {
        ...
        } else if (dietRes.data.length > 0) {
          // 🔥 优先级2：如果没有专用配置但有数据，根据数据构建配置
          console.log('✅ 没有专用配置但有数据，根据数据构建配置');
          const bloodData = dietRes.data[0];
          console.log('📋 饮食数据字段:', Object.keys(bloodData));
      ...
      if (basicIndicators.length === 0 && customIndicators.length === 0) {
        ...
        const fallbackIndicators = [
          { id: 'diet', name: '饮食', min: '1', max: '10', unit: '份' }
        ];
  ...
  async loadTodayData() {
    ...
    console.log(`📊📊📊 开始加载 ${selectedDate} 的饮食数据 📊📊📊`);
      ...
      if (res.data.length > 0) {
        ...
        allConfiguredIds.forEach(indicatorId => {
          if (formData[indicatorId] === undefined) {
            // 🔥 修复：正确处理0值，不能用 || 运算符
            // 🔧 兼容旧字段名
            const fieldValue = (data[indicatorId] || data['fbg'] || data['pbg'] || data['hba1c'] || data['rbg']);
        ...
        console.log(`❌ ${selectedDate} 没有饮食数据，开始清空表单`);

        console.log(`❌ ${selectedDate} 没有饮食数据，清空表单但保持配置`);
  ...
  async loadRecordData(recordId) {
    ...
    try {
      const db = wx.cloud.database();
      const res = await db.collection('diets').doc(recordId).get();
