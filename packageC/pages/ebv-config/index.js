...
Page({
  data: {
    ...
    // 所有指标的配置信息（只保留必选项）
    indicatorConfigs: {
      ebvDna: { name: '数值', min: '', max: '', unit: 'IU/mL' }
    },
  ...
  onLoad(options) {
    ...
    if (userInfoSuccess) {
      ...
      if (app.globalData && app.globalData.temporaryEbvConfig &&
        app.globalData.temporaryEbvConfig.configDate === editingDate) {

        this.loadTemporaryEbvConfigToPage();
      } else {
