...
Page({
  data: {
    ...
    // 必选指标（不能取消勾选）- 针对移植后患者
    requiredIndicators: ['hcmvDna'],

    // 所有指标的配置信息（包括自定义设置）
    indicatorConfigs: {
      hcmvDna: { name: '数值', min: '', max: '', unit: 'IU/mL' },
      pp65: { name: 'PP65抗原', min: '', max: '', unit: '个/10万细胞' },
      igM: { name: '巨细胞病毒IgM', min: '', max: '', unit: 'COI' },
      igG: { name: '巨细胞病毒IgG', min: '', max: '', unit: 'COI' },
      avidity: { name: 'IgG亲和力', min: '', max: '', unit: '%' },
      immediate: { name: '即早基因', min: '', max: '', unit: 'COI' },
      late: { name: '晚期基因', min: '', max: '', unit: 'COI' },
  ...
  onLoad(options) {
    ...
    if (userInfoSuccess) {
      ...
      if (app.globalData && app.globalData.temporaryCmvConfig &&
        app.globalData.temporaryCmvConfig.configDate === editingDate) {

        this.loadTemporaryCmvConfigToPage();
      } else {
  ...
  onShow() {
    ...
    const app = getApp();
    const hasTemporaryConfig = app.globalData && app.globalData.temporaryCmvConfig && app.globalData.temporaryCmvConfig.isTemporary;
  ...
  async loadIndicatorConfig() {
    ...
    try {
      ...
      if (editingDate) {
        // 🔧 优先查询当前日期的专用配置
        configRes = await db.collection('cmvIndicatorConfig')
          .where({
            openid: openid,
        ...
        if (configRes.data.length === 0 && this.data.dateType === 'future') {
          ...
          // 查找最近的有效配置（包括今日和历史配置）
          const inheritableConfigRes = await db.collection('cmvIndicatorConfig')
            .where({
              openid: openid,
              profileId: currentProfileId,
  ...
  syncConfigToCmvPage() {
    ...
    if (app.globalData) {
      ...
      app.globalData.temporaryCmvConfig = {
        selectedIndicators: JSON.parse(JSON.stringify(selectedIndicators)),
        customIndicators: JSON.parse(JSON.stringify(customIndicators)),
        indicatorConfigs: JSON.parse(JSON.stringify(indicatorConfigs)),
  ...
  async saveConfigSimple() {
    ...
    try {
      ...
      // 1. 获取旧配置，找出被取消勾选的指标
      const existingRes = await db.collection('cmvIndicatorConfig')
        .where({
          openid: openid,
          profileId: currentProfileId,
          date: editingDate
      ...
      for (const indicatorId of removedIndicators) {
        ...
        if (settingsRes.data.length > 0) {
          for (const setting of settingsRes.data) {
            await db.collection('cmvIndicatorSettings')
              .doc(setting._id)
              .remove();
          }
    ...
    try {
      ...
      if (isAddMode) {
        ...
        try {
          ...
          existingRes = await db.collection('cmvIndicatorSettings')
            .where({
              openid: openid,
              profileId: currentProfileId,
              indicatorId: targetIndicatorId
      ...
      if (!collectionExists) {
        ...
        try {
          // 通过添加第一条记录来创建集合
          dbResult = await db.collection('cmvIndicatorSettings').add({
            data: {
              openid: openid,
              profileId: currentProfileId,
              indicatorId: targetIndicatorId,
        ...
        try {
          dbResult = await db.collection('cmvIndicatorSettings')
            .doc(existingRes.data[0]._id)
            .update({
              data: settingData
            });
