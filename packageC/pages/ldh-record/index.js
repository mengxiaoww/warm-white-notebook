...
Page({
  data: {
    ...
    // 指标配置和显示 - 乳酸脱氢酶相关指标
    basicIndicators: [
      { id: 'ldh', name: '数值', min: '', max: '', unit: 'U/L' }
    ],
  ...
  onShow() {
    ...
    if (app.globalData) {
      // 检查临时配置（从配置页面返回）
      if (app.globalData.temporaryLdhConfig) {
      ...
      // 检查配置刷新标志
      else if (app.globalData.needRefreshLdhConfig) {

        app.globalData.needRefreshLdhConfig = false;
  ...
  cleanupTemporaryConfigIfNotSaved() {
    const app = getApp();
    if (app.globalData && app.globalData.temporaryLdhConfig && this.data.isTemporaryConfig) {
      ...
      if (currentPage && currentPage.route === 'packageC/pages/ldh-record/index') {
  ...
  async loadIndicatorConfiguration() {
    ...
    try {
      ...
      // 1. 查询当前日期的特定配置
      const configRes = await db.collection('ldhIndicatorConfig')
        .where({
          openid: openid,
      ...
      // 3. 确定最终配置
      let selectedIndicators = { ldh: true }; // 默认只选LDH
      ...
      if (configRes.data.length > 0) {
        ...
        // 如果没有当前日期配置，尝试继承最近配置
        const inheritedConfigRes = await db.collection('ldhIndicatorConfig')
          .where({
            openid: openid,
            profileId: currentProfileId,
  ...
  configIndicators() {
    ...
    // 传递当前日期给配置页面
    wx.navigateTo({
      url: `/packageC/pages/ldh-config/index?date=${selectedDate}`,
      fail: (err) => {
  ...
  async saveTemporaryConfigIfExists() {
    ...
    try {
      ...
      for (const indicator of newCustomIndicators) {
        // 检查是否已存在
        const existingRes = await db.collection('ldhIndicatorSettings')
          .where({
            openid: openid,
            profileId: currentProfileId,
        ...
        if (existingRes.data.length === 0) {
          // 新增自定义指标
          await db.collection('ldhIndicatorSettings').add({
            data: {
              openid: openid,
              profileId: currentProfileId,
              indicatorId: indicator.id,
      ...
      // 查询现有配置
      const existingConfigRes = await db.collection('ldhIndicatorConfig')
        .where({
          openid: openid,
          profileId: currentProfileId,
          date: selectedDate
      ...
      if (app.globalData) {
        app.globalData.needRefreshLdhConfig = true;
      }
  ...
  async saveData() {
    ...
    try {
      ...
      // 检查是否已存在记录
      const existingRes = await db.collection('ldhRecords')
        .where({
          openid: openid,
          profileId: currentProfileId,
          date: selectedDate
      ...
      if (isNewRecord) {
        ...
        await db.collection('ldhRecords').add({
          data: saveData
        });
        ...
        await db.collection('ldhRecords')
          .doc(targetRecordId)
          .update({
            data: updateData
          });
  ...
  fuzzyMatch(aiLabel, configName) {
    ...
    // LDH相关的特殊匹配规则
    const ldhKeywords = [
      ['ldh', '乳酸脱氢酶', 'lactate', 'dehydrogenase', '乳酸', '脱氢酶'],
      ['ldh1', 'ldh-1', 'ldh1同工酶', 'ldh1 isoenzyme'],
      ['ldh2', 'ldh-2', 'ldh2同工酶', 'ldh2 isoenzyme'],
      ['ldh3', 'ldh-3', 'ldh3同工酶', 'ldh3 isoenzyme'],
      ['ldh4', 'ldh-4', 'ldh4同工酶', 'ldh4 isoenzyme'],
      ['ldh5', 'ldh-5', 'ldh5同工酶', 'ldh5 isoenzyme'],
      ['hbdh', 'α-hbdh', 'α羟丁酸脱氢酶', 'hydroxybutyrate'],
      ['ldh比值', 'ldh ratio', '比值'],
      ['总ldh', 'total ldh', '总量']
    ];

    for (const keywords of ldhKeywords) {
