...
Page({
  data: {
    ...
    // 必选指标（不能取消勾选）- 针对移植后患者
    requiredIndicators: ['ldh'],

    // 所有指标的配置信息（包括自定义设置）
    indicatorConfigs: {
      ldh: { name: '数值', min: '', max: '', unit: 'U/L' }
    },
  ...
  onLoad(options) {
    ...
    if (userInfoSuccess) {
      ...
      const app = getApp();
      if (app.globalData && app.globalData.temporaryLdhConfig &&
        app.globalData.temporaryLdhConfig.configDate === editingDate) {

        this.loadTemporaryLdhConfigToPage();
      } else {
  ...
  onHide() {
    ...
    // 🔧 修复：确保在页面隐藏时同步配置到巨细胞病毒页面
    // 无论用户如何离开页面（点击返回按钮、手势返回、系统返回），都同步配置
    this.syncConfigToCmvPage();
  },
  ...
  cleanupTemporaryConfig() {
    ...
    if (app.globalData && app.globalData.temporaryLdhConfig) {
      ...
      delete app.globalData.temporaryLdhConfig;
    }
  },
  ...
  ensureRefreshFlag() {
    const app = getApp();
    if (app.globalData) {
      // 只要页面被访问过，就设置刷新标志，确保巨细胞病毒页面会重新加载
  ...
  onShow() {
    ...
    const app = getApp();
    const hasTemporaryConfig = app.globalData && app.globalData.temporaryLdhConfig && app.globalData.temporaryLdhConfig.isTemporary;
  ...
  // 🆕 处理巨细胞病毒数据删除事件
  handleCmvDataDeleted() {
    const app = getApp();
    if (app.globalData && app.globalData.cmvDataDeleted) {
      const deleteInfo = app.globalData.cmvDataDeleted;
      ...
      if (!isValid) {
        ...
        delete app.globalData.cmvDataDeleted;
        return;
      }
  ...
  shouldUseTemporaryConfig() {
    const app = getApp();
    const tempConfig = app.globalData.temporaryLdhConfig;
  ...
  async loadIndicatorConfig() {
    ...
    try {
      ...
      if (editingDate) {
        // 🔧 优先查询当前日期的专用配置
        configRes = await db.collection('ldhIndicatorConfig')
          .where({
            openid: openid,
        ...
        if (configRes.data.length === 0 && this.data.dateType === 'future') {
          ...
          // 查找最近的有效配置（包括今日和历史配置）
          const inheritableConfigRes = await db.collection('ldhIndicatorConfig')
            .where({
              openid: openid,
              profileId: currentProfileId,
      ...
      let customSettings = {};
      try {
        const settingsRes = await db.collection('ldhIndicatorSettings')
          .where({
            openid: openid,
            profileId: currentProfileId
          })
  ...
  // 返回方法 - 立即同步配置到巨细胞病毒页面预览
  // 返回方法 - 立即同步配置到巨细胞病毒页面预览
  async goBack() {


    // 立即同步当前配置状态到全局数据，供巨细胞病毒页面预览
    this.syncConfigToCmvPage();
  ...
  syncConfigToCmvPage() {
    ...
    const app = getApp();
    if (app.globalData) {
      // 设置临时配置数据，供巨细胞病毒页面预览使用
      app.globalData.temporaryLdhConfig = {
        selectedIndicators: JSON.parse(JSON.stringify(selectedIndicators)),
        customIndicators: JSON.parse(JSON.stringify(customIndicators)),
        indicatorConfigs: JSON.parse(JSON.stringify(indicatorConfigs)),
  ...
  async saveConfigSimple() {
    ...
    try {
      ...
      // 1. 获取旧配置，找出被取消勾选的指标
      const existingRes = await db.collection('ldhIndicatorConfig')
        .where({
          openid: openid,
          profileId: currentProfileId,
          date: editingDate
      ...
      // 3. 保存当前日期的配置
      if (existingRes.data.length > 0) {
        await db.collection('ldhIndicatorConfig')
          .doc(existingRes.data[0]._id)
          .update({
            data: configData
          });
      } else {
        await db.collection('ldhIndicatorConfig').add({
          data: {
            openid: openid,
            profileId: currentProfileId,
            date: editingDate,
  ...
  async applyConfigToFuture(selectedIndicators) {
    ...
    try {
      ...
      // 查询所有未来日期的配置（大于今天的日期）
      const futureConfigsRes = await db.collection('ldhIndicatorConfig')
        .where({
          openid: openid,
          profileId: currentProfileId,
      ...
      const batchUpdatePromises = futureConfigsRes.data.map(config => {
        ...
        return db.collection('ldhIndicatorConfig')
          .doc(config._id)
          .update({
            data: {
              selectedIndicators: selectedIndicators,
      ...
      for (const indicatorId of removedIndicators) {
        ...
        if (settingsRes.data.length > 0) {
          for (const setting of settingsRes.data) {
            await db.collection('ldhIndicatorSettings')
              .doc(setting._id)
              .remove();
          }
  ...
  async cleanupRemovedIndicatorData(removedIndicators, date) {
    ...
    try {
      ...
      // 查询该日期的巨细胞病毒数据
      const cmvRes = await db.collection('cmvRecords')
        .where({
          openid: openid,
          profileId: currentProfileId,
          date: date
      ...
      if (cmvRes.data.length > 0) {
        const cmvData = cmvRes.data[0];
        const updateData = { updateTime: db.serverDate() };
        let needUpdate = false;

        // 清理直接字段
        removedIndicators.forEach(indicatorId => {
          if (cmvData[indicatorId] !== undefined) {
        ...
        if (cmvData.customValues) {
          const newCustomValues = {};
          Object.keys(cmvData.customValues).forEach(key => {
            if (!removedIndicators.includes(key)) {
              newCustomValues[key] = cmvData.customValues[key];
            }
          });
  ...
  async editIndicator(e) {
    ...
    const defaultIndicators = {
      ...
      igA: { name: 'IgA抗体', min: '', max: '', unit: 'COI' },
      antigenP52: { name: 'P52抗原', min: '', max: '', unit: 'COI' },
      earlyAntigen: { name: '早期抗原', min: '', max: '', unit: 'COI' }
    };
  ...
  generateAutoShortName(fullName) {
    ...
    if (name.length === 1) {
      ...
      if (/[\u4e00-\u9fa5]/.test(name)) {
        ...
        } else if (name.includes('抗原')) {
          // P52抗原 -> P52
          shortName = name.replace('抗原', '').substring(0, 3);
        } else {
          // 通用策略：取前两个字符
  ...
  checkShortNameConflict(shortName) {
    ...
    // 预设指标的简称
    const presetShortNames = ['IgM', 'IgG', 'DNA', 'PP65', 'IgA', 'P52', '早期'];
    ...
    try {
      ...
      if (isAddMode) {
        ...
        try {
          ...
          existingRes = await db.collection('ldhIndicatorSettings')
            .where({
              openid: openid,
              profileId: currentProfileId,
              indicatorId: targetIndicatorId
      ...
      if (!collectionExists) {
        ...
        try {
          // 通过添加第一条记录来创建集合
          dbResult = await db.collection('ldhIndicatorSettings').add({
            data: {
              openid: openid,
              profileId: currentProfileId,
              indicatorId: targetIndicatorId,
        ...
        try {
          dbResult = await db.collection('ldhIndicatorSettings')
            .doc(existingRes.data[0]._id)
            .update({
              data: settingData
            });
