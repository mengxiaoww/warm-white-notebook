...
Page({
  data: {
    ...
    // 必选指标（不能取消勾选）
    requiredIndicators: ['temperature'],
  ...
  onLoad(options) {
    ...
    if (userInfoSuccess) {
      ...
      const app = getApp();
      if (app.globalData && app.globalData.temporaryTemperatureConfig &&
        app.globalData.temporaryTemperatureConfig.configDate === editingDate) {
  ...
  onShow() {
    ...
    const app = getApp();
    const hasTemporaryConfig = app.globalData && app.globalData.temporaryTemperatureConfig && app.globalData.temporaryTemperatureConfig.isTemporary;
  ...
  handleBloodTestDataDeleted() {
    const app = getApp();
    if (app.globalData && app.globalData.temperatureDataDeleted) {
      const deleteInfo = app.globalData.temperatureDataDeleted;
      ...
      if (!isValid) {
        ...
        delete app.globalData.temperatureDataDeleted;
        return;
      }
  ...
  async loadIndicatorConfig() {
    ...
    try {
      ...
      if (editingDate) {
        // 🔧 优先查询当前日期的专用配置
        configRes = await db.collection('temperatureIndicatorConfig')
          .where({
            openid: openid,
        ...
        if (configRes.data.length === 0 && this.data.dateType === 'future') {
          ...
          // 查找最近的有效配置（包括今日和历史配置）
          const inheritableConfigRes = await db.collection('temperatureIndicatorConfig')
            .where({
              openid: openid,
              profileId: currentProfileId,
      ...
      let customSettings = {};
      try {
        const settingsRes = await db.collection('temperatureIndicatorSettings')
          .where({
            openid: openid,
            profileId: currentProfileId
          })
      ...
      if (configRes.data.length > 0) {
        ...
        // 🔧 兼容旧字段名：temp -> temperature
        if (config.selectedIndicators && config.selectedIndicators.temp !== undefined) {
          finalSelectedIndicators.temperature = config.selectedIndicators.temp;
        }
  ...
  async loadCustomIndicators() {
    ...
    try {
      const db = wx.cloud.database();
      const res = await db.collection('temperatureIndicators')
        .where({
          openid: openid,
          profileId: currentProfileId
        })
  ...
  syncConfigToBloodTestPage() {
    ...
    if (app.globalData) {
      // 设置临时配置数据，供体温页面预览使用
      app.globalData.temporaryTemperatureConfig = {
        selectedIndicators: JSON.parse(JSON.stringify(selectedIndicators)),
        customIndicators: JSON.parse(JSON.stringify(customIndicators)),
        indicatorConfigs: JSON.parse(JSON.stringify(indicatorConfigs)),
  ...
  async updateCustomIndicatorStatus(indicatorId, isActive) {
    try {
      const db = wx.cloud.database();
      await db.collection('temperatureIndicators')
        .doc(indicatorId)
        .update({
          data: {
            isActive: isActive,
  ...
  async saveConfigSimple() {
    ...
    try {
      ...
      // 1. 获取旧配置，找出被取消勾选的指标
      const existingRes = await db.collection('temperatureIndicatorConfig')
        .where({
          openid: openid,
          profileId: currentProfileId,
          date: editingDate
      ...
      for (const indicatorId of removedIndicators) {
        ...
        if (settingsRes.data.length > 0) {
          for (const setting of settingsRes.data) {
            await db.collection('temperatureIndicatorSettings')
              .doc(setting._id)
              .remove();
          }
  ...
  async cleanupRemovedIndicatorData(removedIndicators, date) {
    ...
    try {
      ...
      // 查询该日期的体温数据
      const bloodRes = await db.collection('temperatures')
        .where({
          openid: openid,
          profileId: currentProfileId,
          date: date
  ...
  async saveConfig() {
    ...
    try {
      ...
      // 查询是否已有配置
      const res = await db.collection('temperatureIndicatorConfig')
        .where({
          openid: openid,
          profileId: currentProfileId
        })
      ...
      if (res.data.length > 0) {
        ...
        // 更新配置
        const updateResult = await db.collection('temperatureIndicatorConfig')
          .doc(res.data[0]._id)
          .update({
            data: configData
          });
        ...
        // 新建配置
        const addResult = await db.collection('temperatureIndicatorConfig').add({
          data: {
            openid,
            profileId: currentProfileId,
            selectedIndicators,
    ...
    try {
      ...
      if (isAddMode) {
        ...
        try {
          ...
          existingRes = await db.collection('temperatureIndicatorSettings')
            .where({
              openid: openid,
              profileId: currentProfileId,
              indicatorId: targetIndicatorId
      ...
      if (!collectionExists) {
        ...
        try {
          // 通过添加第一条记录来创建集合
          dbResult = await db.collection('temperatureIndicatorSettings').add({
            data: {
              openid: openid,
              profileId: currentProfileId,
              indicatorId: targetIndicatorId,
        ...
        try {
          dbResult = await db.collection('temperatureIndicatorSettings')
            .doc(existingRes.data[0]._id)
            .update({
              data: settingData
            });
