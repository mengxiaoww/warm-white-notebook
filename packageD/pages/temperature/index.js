...
Page({
  ...
  // 页面初始化
  onLoad(options) {
    console.log('🎯 体温页面 onLoad，传入参数:', options);
  ...
  onShow() {
    ...
    // 检查是否有临时配置预览（从配置页面返回）
    if (app.globalData.temporaryTemperatureConfig) {
      this.loadTemporaryConfiguration();

      // 🔧 关键修复：恢复用户之前的输入
      this.restoreTemporaryUserInput();
    } else if (app.globalData.needRefreshTemperatureConfig || app.globalData.indicatorConfigChanged) {
  ...
  loadTemporaryConfiguration() {
    const app = getApp();
    const tempConfig = app.globalData.temporaryTemperatureConfig;
  ...
  restoreTemporaryUserInput() {
    ...
    if (tempInput.selectedDate === this.data.selectedDate) {
      console.log('🔄 恢复体温用户临时输入:', tempInput.formData);
      ...
      console.log('✅ 体温用户输入已恢复');
    } else {
      console.log('📅 体温临时输入日期不匹配，忽略恢复');
    }
  ...
  cleanupTemporaryConfigIfNotSaved() {
    const app = getApp();
    if (app.globalData && app.globalData.temporaryTemperatureConfig && this.data.isTemporaryConfig) {
      console.log('🧹 清理未保存的临时配置');
      delete app.globalData.temporaryTemperatureConfig;
  ...
  parseOCRResult(items) {
    ...
    items.forEach((item, index) => {
      ...
      // 特别标记包含体温相关内容的行
      if (text.includes('体温') || text.includes('FBG') || text.includes('PBG') || text.includes('HbA1c')) {
        console.log(`    ⚠️ 此行包含体温相关关键词`);
      }
    });
  ...
  shouldSkipPercentage(keyword, text) {
    // 体温指标不需要跳过百分比,因为HbA1c本身就是百分比
    // 只需要确保不误匹配其他无关的百分比数据
    return false;
  },

  // 重新设计的数值提取逻辑 - 针对5张体温图片优化
      ...
      for (let searchOffset = 0; searchOffset <= 5; searchOffset++) {
        ...
        if (fieldName === 'crp') {
          ...
          if (searchText.includes('检查项目') || searchText.includes('设备') ||
            ...
            searchText.includes('血球仪') || searchText.includes('全体温') ||
  ...
  configIndicators() {
    ...
    // 🔧 重要：传递当前日期给配置页面
    wx.navigateTo({
      url: `/packageD/pages/temperature-config/index?date=${selectedDate}`,
      fail: (err) => {
  ...
  viewHistory() {
    wx.navigateTo({
      url: '/pages/blood-sugar-history/index'
    });
  },
  ...
  async saveData() {
    ...
    try {
      ...
      const existingRes = await db.collection('temperatures')
        .where({
          openid: openid,
      ...
      if (isNewRecord) {
        ...
        const res = await db.collection('temperatures').add({
          data: saveData
        });
        ...
        await db.collection('temperatures')
          .doc(targetRecordId)
          .update({
            data: updateData
          });
  ...
  async findInheritedConfig(targetDate, openid, currentProfileId) {
    try {
      ...
      const configRes = await db.collection('temperatureIndicatorConfig')
        .where({
          openid: openid,
          profileId: currentProfileId,
  ...
  async saveTemporaryConfigIfExists() {
    ...
    try {
      ...
      // 查询现有配置
      const existingConfigRes = await db.collection('temperatureIndicatorConfig')
        .where({
          openid: openid,
          profileId: currentProfileId,
          date: selectedDate
      ...
      if (existingConfigRes.data.length > 0) {
        // 更新现有配置
        await db.collection('temperatureIndicatorConfig')
        ...
        // 创建新配置
        await db.collection('temperatureIndicatorConfig').add({
          data: {
            openid: openid,
            profileId: currentProfileId,
            date: selectedDate,
      ...
      if (app.globalData) {
        app.globalData.needRefreshTemperatureConfig = true;
  ...
  async applyTodayConfigToFuture(todayDate, selectedIndicators, openid, currentProfileId, db) {
    try {
      // 查找所有未来日期的配置
      const futureConfigsRes = await db.collection('temperatureIndicatorConfig')
        .where({
          openid: openid,
          profileId: currentProfileId,
      ...
      const batchUpdatePromises = futureConfigsRes.data.map(config => {
        ...
        return db.collection('temperatureIndicatorConfig')
          .doc(config._id)
          .update({
            data: {
              selectedIndicators: selectedIndicators,
  ...
  async deleteRecord() {
    ...
    try {
      ...
      wx.showModal({
        title: '确认删除',
        content: `确定要删除 ${selectedDate} 的体温记录吗？`,
        confirmText: '删除',
        cancelText: '取消',
        success: async (modalRes) => {
          ...
          if (modalRes.confirm) {
            try {
              // 1. 删除体温记录
              await db.collection('temperatures').doc(actualRecordId).remove();
  ...
  async loadCompleteConfiguration() {
    ...
    try {
      ...
      const currentDateConfigRes = await db.collection('temperatureIndicatorConfig')
        .where({
          openid: openid,
          profileId: currentProfileId,
          date: selectedDate
      ...
      // 2. 查询当前日期的体温数据
      console.log('🔍 步骤2：查询当前日期的体温数据');
      console.log('🔍 查询参数确认:', { openid, profileId: currentProfileId, date: selectedDate });

      const temperatureRes = await db.collection('temperatures')
        .where({
          openid: openid,
          profileId: currentProfileId,
          date: selectedDate
      ...
      console.log('📋 体温数据查询结果:', temperatureRes.data.length, '条记录');
      if (temperatureRes.data.length > 0) {
        console.log('📋 找到的数据:', temperatureRes.data[0]);
      }
      ...
      try {
        customSettingsRes = await db.collection('temperatureIndicatorSettings')
          .where({
            openid: openid,
            profileId: currentProfileId
          })
      ...
      console.log(`  - 体温数据: ${temperatureRes.data ? temperatureRes.data.length : '查询失败'} 条`);
      ...
      if (!temperatureRes || !temperatureRes.data || !Array.isArray(temperatureRes.data)) {
        console.error('❌ 体温数据查询结果格式异常:', temperatureRes);
        throw new Error('体温数据查询结果格式异常');
      }
      ...
      if (currentDateConfigRes.data.length > 0) {
        ...
        } else if (temperatureRes.data.length > 0) {
          // 🔥 优先级2：如果没有专用配置但有数据，根据数据构建配置
          console.log('✅ 没有专用配置但有数据，根据数据构建配置');
          const bloodData = temperatureRes.data[0];
          console.log('📋 体温数据字段:', Object.keys(bloodData));
      ...
      if (basicIndicators.length === 0 && customIndicators.length === 0) {
        ...
        const fallbackIndicators = [
          { id: 'temperature', name: '体温', min: '36.0', max: '37.3', unit: '°C' }
        ];
      ...
      this.setData({
        temperatureIndicatorConfig: config,
  ...
  async loadTodayData() {
    ...
    console.log(`📊📊📊 开始加载 ${selectedDate} 的体温数据 📊📊📊`);
    ...
    try {
      ...
      if (res.data.length > 0) {
        ...
        console.log(`❌ ${selectedDate} 没有体温数据，开始清空表单`);

        console.log(`❌ ${selectedDate} 没有体温数据，清空表单但保持配置`);
  ...
  async loadRecordData(recordId) {
    ...
    try {
      const db = wx.cloud.database();
      const res = await db.collection('temperatures').doc(recordId).get();
