...
Page({
  ...
  // 页面初始化
  onLoad(options) {
    console.log('🎯 饮水页面 onLoad，传入参数:', options);
  ...
  onShow() {
    ...
    // 检查是否有临时配置预览（从配置页面返回）
    if (app.globalData.temporaryWaterIntakeConfig) {
      this.loadTemporaryConfiguration();

      // 🔧 关键修复：恢复用户之前的输入
      this.restoreTemporaryUserInput();
    } else if (app.globalData.needRefreshWaterIntakeConfig || app.globalData.indicatorConfigChanged) {
      app.globalData.needRefreshWaterIntakeConfig = false;
  ...
  restoreTemporaryUserInput() {
    ...
    if (tempInput.selectedDate === this.data.selectedDate) {
      console.log('🔄 恢复饮水用户临时输入:', tempInput.formData);
      ...
      console.log('✅ 饮水用户输入已恢复');
    } else {
      console.log('📅 饮水临时输入日期不匹配，忽略恢复');
    }
  ...
  cleanupTemporaryConfigIfNotSaved() {
    const app = getApp();
    if (app.globalData && app.globalData.temporaryWaterIntakeConfig && this.data.isTemporaryConfig) {
  ...
  parseOCRResult(items) {
    ...
    items.forEach((item, index) => {
      ...
      // 特别标记包含饮水相关内容的行
      if (text.includes('饮水') || text.includes('FBG') || text.includes('PBG') || text.includes('HbA1c')) {
        console.log(`    ⚠️ 此行包含饮水相关关键词`);
      }
    });
  ...
  shouldSkipPercentage(keyword, text) {
    // 饮水指标不需要跳过百分比,因为HbA1c本身就是百分比
    // 只需要确保不误匹配其他无关的百分比数据
    return false;
  },

  // 重新设计的数值提取逻辑 - 针对5张饮水图片优化
      ...
      for (let searchOffset = 0; searchOffset <= 5; searchOffset++) {
        ...
        if (fieldName === 'crp') {
          ...
          if (searchText.includes('检查项目') || searchText.includes('设备') ||
            ...
            searchText.includes('血球仪') || searchText.includes('全饮水') ||
  ...
  configIndicators() {
    ...
    // 🔧 重要：传递当前日期给配置页面
    wx.navigateTo({
      url: `/packageD/pages/water-intake-config/index?date=${selectedDate}`,
      fail: (err) => {
  ...
  async saveData() {
    ...
    try {
      ...
      const existingRes = await db.collection('waterIntakes')
        .where({
          openid: openid,
      ...
      if (isNewRecord) {
        ...
        const res = await db.collection('waterIntakes').add({
          data: saveData
        });
        ...
        await db.collection('waterIntakes')
          .doc(targetRecordId)
          .update({
            data: updateData
          });
  ...
  async findInheritedConfig(targetDate, openid, currentProfileId) {
    try {
      ...
      const configRes = await db.collection('waterIntakeIndicatorConfig')
        .where({
          openid: openid,
          profileId: currentProfileId,
  ...
  async saveTemporaryConfigIfExists() {
    ...
    try {
      ...
      for (const indicator of newCustomIndicators) {
        // 检查是否已存在
        const existingRes = await db.collection('waterIntakeIndicatorSettings')
          .where({
            openid: openid,
            profileId: currentProfileId,
        ...
        if (existingRes.data.length === 0) {
          ...
          await db.collection('waterIntakeIndicatorSettings').add({
            data: {
              openid: openid,
              profileId: currentProfileId,
              indicatorId: indicator.id,
      ...
      // 查询现有配置
      const existingConfigRes = await db.collection('waterIntakeIndicatorConfig')
        .where({
          openid: openid,
          profileId: currentProfileId,
          date: selectedDate
      ...
      if (existingConfigRes.data.length > 0) {
        // 更新现有配置
        await db.collection('waterIntakeIndicatorConfig')
        ...
        // 创建新配置
        await db.collection('waterIntakeIndicatorConfig').add({
          data: {
            openid: openid,
            profileId: currentProfileId,
            date: selectedDate,
  ...
  async applyTodayConfigToFuture(todayDate, selectedIndicators, openid, currentProfileId, db) {
    try {
      // 查找所有未来日期的配置
      const futureConfigsRes = await db.collection('waterIntakeIndicatorConfig')
        .where({
          openid: openid,
          profileId: currentProfileId,
      ...
      const batchUpdatePromises = futureConfigsRes.data.map(config => {
        ...
        return db.collection('waterIntakeIndicatorConfig')
          .doc(config._id)
          .update({
            data: {
              selectedIndicators: selectedIndicators,
  ...
  async deleteRecord() {
    ...
    try {
      ...
      wx.showModal({
        title: '确认删除',
        content: `确定要删除 ${selectedDate} 的饮水记录吗？`,
        confirmText: '删除',
        cancelText: '取消',
        success: async (modalRes) => {
          ...
          if (modalRes.confirm) {
            try {
              // 1. 删除饮水记录
              await db.collection('waterIntakes').doc(actualRecordId).remove();
  ...
  async loadCompleteConfiguration() {
    ...
    try {
      ...
      const currentDateConfigRes = await db.collection('waterIntakeIndicatorConfig')
        .where({
          openid: openid,
          profileId: currentProfileId,
          date: selectedDate
      ...
      // 2. 查询当前日期的饮水数据
      console.log('🔍 步骤2：查询当前日期的饮水数据');
      console.log('🔍 查询参数确认:', { openid, profileId: currentProfileId, date: selectedDate });

      const waterIntakeRes = await db.collection('waterIntakes')
        .where({
          openid: openid,
          profileId: currentProfileId,
          date: selectedDate
      ...
      console.log('📋 饮水数据查询结果:', waterIntakeRes.data.length, '条记录');
      if (waterIntakeRes.data.length > 0) {
        console.log('📋 找到的数据:', waterIntakeRes.data[0]);
      }
      ...
      try {
        customSettingsRes = await db.collection('waterIntakeIndicatorSettings')
          .where({
            openid: openid,
            profileId: currentProfileId
          })
      ...
      console.log(`  - 饮水数据: ${waterIntakeRes.data ? waterIntakeRes.data.length : '查询失败'} 条`);
      ...
      if (!waterIntakeRes || !waterIntakeRes.data || !Array.isArray(waterIntakeRes.data)) {
        console.error('❌ 饮水数据查询结果格式异常:', waterIntakeRes);
        throw new Error('饮水数据查询结果格式异常');
      }
      ...
      if (currentDateConfigRes.data.length > 0) {
        ...
        } else if (waterIntakeRes.data.length > 0) {
          // 🔥 优先级2：如果没有专用配置但有数据，根据数据构建配置
          console.log('✅ 没有专用配置但有数据，根据数据构建配置');
          const bloodData = waterIntakeRes.data[0];
          console.log('📋 饮水数据字段:', Object.keys(bloodData));
      ...
      this.setData({
        waterIntakeIndicatorConfig: config,
  ...
  async loadTodayData() {
    ...
    console.log(`📊📊📊 开始加载 ${selectedDate} 的饮水数据 📊📊📊`);
    ...
    try {
      ...
      if (res.data.length > 0) {
        ...
        console.log(`❌ ${selectedDate} 没有饮水数据，开始清空表单`);

        console.log(`❌ ${selectedDate} 没有饮水数据，清空表单但保持配置`);
  ...
  async loadRecordData(recordId) {
    ...
    try {
      const db = wx.cloud.database();
      const res = await db.collection('waterIntakes').doc(recordId).get();
