...
const { navigateToFunction } = require('../../utils/route-config.js');
...
Page({
  ...
  // 页面导航 - 已统一到 utils/route-config.js
  ...
  handleFunctionTap(e) {
    ...
    if (functionId) {
      const selectedDate = this.data.selectedDate
      navigateToFunction(functionId, selectedDate)
    } else {
      console.error('功能项ID不存在:', functionId)
    }
